...
node {
  ...
  if (env.BRANCH_NAME == 'tmp-jenkins-hitl-safety') {
    deviceStage("safety panda", "tizi-common", ["UNSAFE=1"], [
      step("safety panda", "SAFETY_TEST_NAME=opendbc_repo.opendbc.safety.tests.test_chrysler.TestChryslerRamDTSafety.test_realtime_limit_up openpilot/selfdrive/test/test_safety_panda.sh", [timeout: 86000]),
    ])
    return
  }
